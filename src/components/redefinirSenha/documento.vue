<template>
  <div id="documento" class="form-redefinir-senha container-fluid">
    <!-- <p>{{$v}}</p> -->
    <!-- debug -->
    ...
    <form @submit.prevent="enviaInfo" class="row">
      ...
      <div class="form-group col-xs-12" :class="{'form-invalid': $v.userDocument.$invalid && $v.userDocument.required}">
        <label for="documento">{{text.documento}}</label>
        <the-mask id="documento" v-model.trim="$v.userDocument.$model" :mask="['###.###.###-##', '##.###.###/####-##']"></the-mask>            
      </div>
      <div class="form-group col-xs-12"  :class="{'form-invalid': $v.affiliate.$invalid && $v.affiliate.required}">
      ...
      <div class="form-group">
        ...
        <div class="col-xs-12" v-if="submitStatus">
          ...
          <div class="alert alert-success" v-if="submitStatus == 'SUCESSO'">Uma mensagem foi enviada para {{userMaskedEmail}} com as instruções para redefinição de senha.</div>
        </div>
...
export default {
  data() {
    return {
      text: {
        ...
        infoText: "Informe os dados de sua afiliação",
        ou: "OU",
  ...
  methods: {
    enviaInfo() {
      ...
      self.$axios
        .post(`${process.env.API_AFFILIATE}/login/ForgotLogin`, {
