<template>
  <div class="page-interno page-financiamentos">
    <Base-Loading />
    <layout>
      ...
      <conteudo-interno slot="main">
        <div id="conteudo-financiamentos" class="page-financiamentos" slot="content">
...
export default {
  ...
  asyncData({redirect, store}) {
   if (!checkAccess(store, ROLES.FINANCING)) {
      redirect("/painel-do-cliente");
    }
  },
  /* asyncData(context) {
    console.log("[Financiamentos] Context: ", context);

    if (context.route.fullPath == "/financiamentos") {
      context.redirect("/financiamentos/auto/simulacao");
    }
  }, */
};
</script>
