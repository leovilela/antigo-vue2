...
export default {
  data() {
    return {
      text: {
        modalNovaTransacao_title: "Confirmar empréstimo",
        concluirTransacao: "Contratar empréstimo",
        novaTransacao: "Empréstimo",
        label_valorTransacao: "Valor do empréstimo",
        label_valorLiberado: "Valor Liberado",
        label_parcelas: "Parcelas",
        label_bandeira: "Bandeira",
        tentarNovamente: "Tentar novamente",
        transacaoSucesso: "Contratação realizada com sucesso.",
  ...
  methods: {
    ...
    novaTransacao() {
      ...
      if (varconfirm) {
        self.$router.go({ path: "/emprestimo", force: true });
      }
    },
    ...
    concluirTransacao() {
      ...
      const self = this;
      let externalForm;
      self.$axios.post(`${process.env.API_TRANSACTION}/loan/create`, this.transactionObj).then((response) => {
